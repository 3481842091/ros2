...
3. 客户机的 `host` 填**宿主机 VMnet8 地址**（本机为 `192.168.198.1`），不是 `127.0.0.1`
...
其中 `q_Cw` 是世界 NED→ENU 的基变换（绕轴 `(1,1,0)/√2` 转 180°），
...
**① 先在宿主 Windows 上启动模拟器**（工作目录必须是模拟器包根，它要读同目录的 `settings.json`）：
